...
class BookingWindow:
    ...
    def create_ui(self):
        ...
        self.check_in_var = tk.StringVar(value=date.today())

        self.check_in_date_label = ctk.CTkLabel(master=self.frame, text="Check In Date", font=("Arial", 18))
        self.check_in_date_label.place(x=100, y=105)

        self.check_in_date = ctk.CTkEntry(master=self.frame, width=240, height=42, font=("Arial", 16), textvariable=self.check_in_var, placeholder_text="To be displayed here...", state="readonly")
        self.check_in_date.place(x=275, y=100)

        self.check_in_date_icon = ctk.CTkButton(master=self.frame, text="", command=self.load_check_in_window, fg_color="white", hover_color="#D3D3D3", image=self.calendar_image, font=('Arial', 24), width=35, height=25)
        self.check_in_date_icon.place(x=515, y=100)

        self.check_out_var = tk.StringVar(value=date.today())

        self.check_out_date_label = ctk.CTkLabel(master=self.frame, text="Check Out Date", font=("Arial", 18))
        self.check_out_date_label.place(x=100, y=180)

        self.check_out_date = ctk.CTkEntry(master=self.frame, width=240, height=42, font=("Arial", 16), textvariable=self.check_out_var, placeholder_text="To be displayed here...", state="readonly")
        self.check_out_date.place(x=275, y=175)

        self.check_out_date_icon = ctk.CTkButton(master=self.frame, text="", command=self.load_check_out_window, fg_color="white", hover_color="#D3D3D3", image=self.calendar_image, font=('Arial', 24), width=35, height=25)
        self.check_out_date_icon.place(x=515, y=175)
    ...
    def on_check_in_date_select(self, event):
        selected = self.check_in_cal.get_date()
        self.check_in_label.configure(text=f"Selected Date: {selected}")


    def on_check_out_date_select(self, event):
        selected = self.check_out_cal.get_date()
        self.check_out_label.configure(text=f"Selected Date: {selected}")


    def load_check_in_window(self):
        from tkcalendar import Calendar
        import tkinter.ttk as ttk

        self.check_in_window = ctk.CTkToplevel(self.booking_window)
        self.check_in_window.geometry("400x280")
        self.check_in_window.title("Check In Date Selection Window")

        self.frame = ctk.CTkFrame(master=self.check_in_window)
        self.frame.pack(pady=20, padx=60, fill="both", expand=True)
        ...
        self.check_in_cal = Calendar(self.frame, selectmode="day", mindate=date.today())
        self.check_in_cal.pack()

        selected = self.check_in_cal.get_date()
        self.check_in_label = ctk.CTkLabel(self.frame, text=f"Selected Date: {selected}")
        self.check_in_label.pack(pady=10)

        self.select_date_button = ctk.CTkButton(master=self.frame, command=self.validate_check_in_date, text="Select Date", width=100)
        self.select_date_button.pack()

        self.check_in_cal.bind("<<CalendarSelected>>", self.on_check_in_date_select)

        style.theme_use(current_theme)


    def load_check_out_window(self):
        from tkcalendar import Calendar
        import tkinter.ttk as ttk

        self.check_out_window = ctk.CTkToplevel(self.booking_window)
        self.check_out_window.geometry("400x280")
        self.check_out_window.title("Check Out Date Selection Window")

        self.frame = ctk.CTkFrame(master=self.check_out_window)
        self.frame.pack(pady=20, padx=60, fill="both", expand=True)
        ...
        self.check_out_cal = Calendar(self.frame, selectmode="day", mindate=date.today())
        self.check_out_cal.pack()

        selected = self.check_out_cal.get_date()
        self.check_out_label = ctk.CTkLabel(self.frame, text=f"Selected Date: {selected}")
        self.check_out_label.pack(pady=10)

        self.select_date_button = ctk.CTkButton(master=self.frame, command=self.validate_check_out_date, text="Select Date", width=100)
        self.select_date_button.pack()

        self.check_out_cal.bind("<<CalendarSelected>>", self.on_check_out_date_select)

        style.theme_use(current_theme)


    def validate_check_in_date(self):
        selected_date = self.check_in_cal.get_date()
        self.check_in_window.destroy()
        self.check_in_var.set(datetime.strptime(selected_date, "%m/%d/%y").date())


    def validate_check_out_date(self):
        selected_date = self.check_out_cal.get_date()
        self.check_out_window.destroy()
        self.check_out_var.set(datetime.strptime(selected_date, "%m/%d/%y").date())


    def validate_details(self):
        # Checks to see if all fields are valid, if valid sends the details onto the backend
        booking_validated = BookingSystem.validate_details(self.check_in_date.get(), self.check_out_date.get(), self.adults, self.children)
